...
static gboolean
fu_mtd_device_get_is_locked(FuMtdDevice *self, gboolean *locked, GError **error)
{
#ifdef HAVE_MTD_USER_H
	gint rc = 0;
...
static void
fu_mtd_device_security_attr_add_metadata(FuMtdDevice *self, FwupdSecurityAttr *attr)
{
	FuMtdDevicePrivate *priv = GET_PRIVATE(self);
	g_autofree gchar *length_str = NULL;

	fwupd_security_attr_add_metadata(attr, "provider", "mtd");
	fwupd_security_attr_add_metadata(attr, "kernel-api", "MEMISLOCKED");
	if (priv->mtd_type != NULL)
		fwupd_security_attr_add_metadata(attr, "mtd-type", priv->mtd_type);
	length_str = g_strdup_printf("0x%" G_GINT64_MODIFIER "x",
				     fu_device_get_firmware_size_max(FU_DEVICE(self)));
	fwupd_security_attr_add_metadata(attr, "range-start", "0x0");
	fwupd_security_attr_add_metadata(attr, "range-length", length_str);
}
...
fu_mtd_device_add_security_attrs(FuDevice *device, FuSecurityAttrs *attrs)
{
	...
	attr = fu_device_security_attr_new(device, FWUPD_SECURITY_ATTR_ID_SPI_RO_WRITE_PROTECTION);
	fwupd_security_attr_set_result_success(attr, FWUPD_SECURITY_ATTR_RESULT_LOCKED);
	fu_mtd_device_security_attr_add_metadata(self, attr);
	...
	if (!fu_mtd_device_get_is_locked(self, &locked, &error_local)) {
		fwupd_security_attr_set_result(attr, FWUPD_SECURITY_ATTR_RESULT_NOT_SUPPORTED);
		fwupd_security_attr_add_flag(attr, FWUPD_SECURITY_ATTR_FLAG_MISSING_DATA);
...
fu_mtd_device_probe(FuDevice *device, GError **error)
{
	...
	attr_type = fu_udev_device_read_sysfs(FU_UDEV_DEVICE(device),
					      ...
					      NULL);
	if (attr_type != NULL)
		priv->mtd_type = fu_strstrip(g_steal_pointer(&attr_type));
