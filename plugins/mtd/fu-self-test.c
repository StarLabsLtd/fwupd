...
static FuMtdDevice *
fu_test_mtd_device_new_for_security_attrs(FuTest *self, gboolean locked)
{
	gboolean ret;
...
fu_test_mtd_device_security_attrs_locked_func(gconstpointer user_data)
{
	...
	device = fu_test_mtd_device_new_for_security_attrs(self, TRUE);
	fu_device_add_security_attrs(FU_DEVICE(device), attrs);

	attr = fu_security_attrs_get_by_appstream_id(attrs,
						     FWUPD_SECURITY_ATTR_ID_SPI_RO_WRITE_PROTECTION,
						     NULL);
	g_assert_nonnull(attr);
	g_assert_true(fwupd_security_attr_has_flag(attr, FWUPD_SECURITY_ATTR_FLAG_SUCCESS));
	...
	g_assert_cmpstr(fwupd_security_attr_get_metadata(attr, "provider"), ==, "mtd");
	g_assert_cmpstr(fwupd_security_attr_get_metadata(attr, "kernel-api"), ==, "MEMISLOCKED");
	g_assert_cmpstr(fwupd_security_attr_get_metadata(attr, "mtd-type"), ==, "nor");
	g_assert_cmpstr(fwupd_security_attr_get_metadata(attr, "range-start"), ==, "0x0");
	g_assert_cmpstr(fwupd_security_attr_get_metadata(attr, "range-length"), ==, "0x100000");
#endif
}
...
fu_test_mtd_device_security_attrs_unlocked_func(gconstpointer user_data)
{
	...
	device = fu_test_mtd_device_new_for_security_attrs(self, FALSE);
	fu_device_add_security_attrs(FU_DEVICE(device), attrs);
